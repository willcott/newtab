...
- The user changes the goal, workflow, data model, constraints, visual direction, technical shape, or other current product decision.
...
5. Remove or revise obsolete, exclusion-only, comparative, or rationale-only statements when the new specification supersedes them. Do not create or retain `Non-Goals`, `Future Considerations`, or roadmap sections unless the user explicitly asks for them.
...
- Describe the current product requirements, active constraints, and implementation decisions. Do not add absent-feature lists, non-requirements, or future ideas by default.
...
- Prefer direct statements of what the product does. Remove incidental comparisons and exclusions such as `instead of`, `rather than`, `without`, or `no ...` when they only describe what the product does not include. Keep negative wording only when it defines an active product invariant or implementation constraint.
- Before finishing, scan the full brief for non-requirement language and remove or rewrite it while preserving constraints that affect behavior or acceptance.
